...
sub signature {
  my %have = @_;
  join " ",
    map { join(",", split $;) . ":" . $have{$_} }
    sort { $a cmp $b } keys %have
}
...
sub formathave {
  ...
  for $row (0..$width-1) {
    for $column (0..$height-1) {
      $virtual[$row] .= $virtual{$row, $column} ? "O" : $blocks{$row, $column} ? "#" : ".";
      $have[$row] .= $blocks{$row, $column} ? 0 + $have{$row, $column} : ".";
    }
  }
...
for (keys %rocks) {
  my ($row, $column) = split $;;
  my ($nextrow, $nextcolumn) = @{$magnet{$row, $column}{north}};
...
if ($target < $parallel) {
  ...
  my $offsettarget = $target - $parallel - 1;
  my @cyclicalscores = @scores[$parallel..$#scores-1];
  print "Repeating section\n";
  printf "\t%3d. $cyclicalscores[$_] ($digests[$parallel+$_])\n", $_ for (0..$#cyclicalscores);
  printf "Is the answer: %d? ($target %% %d)\n", $cyclicalscores[$offsettarget % @cyclicalscores], scalar @cyclicalscores;
}
