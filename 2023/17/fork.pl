...
sub esotericdumptruck {
  my ($row, $column) = @_;
  my @marks = ([red, 0, 0], [yellow, $row, $column]);
...
search: while (1) {
  ...
  unless ($m % $interval) {
    esotericdumptruck $minimumrow, $minimumcolumn;
    dumptruck;
  }
  if ($minimumrow == $targetrow and $minimumcolumn == $targetcolumn) {
  ...
  if ($minimumcolumn != 0 and $minimumrow != 0) {
    die "sourcerow not defined" unless defined $sourcerow;
    die "sourcecolumn not defined" unless defined $sourcecolumn
  }
  ...
  for my $direction (north, south, east, west) {
    ...
    if ($direction eq $previousdirection  && 3 <= 1 + $stretchTo{$minimumrow, $minimumcolumn}) {
      print("\tCANNOT go straight for longer than three units ($previousdirection → $direction) ($stretch)\n");
      next;
    }
    print("\tIn this case ($previousdirection → $direction → $row, $column) (stretch=$stretch)\n");
 ...
 idekanymore: {
    ...
    if ($dist < $existingdist) {
      ...
      die "bro" if !defined($direction) && (!defined($sourcerow) || $sourcerow != 0) && (!defined($sourcecolumn) || $sourcecolumn != 0);
...
# new strategy: enumerate all paths with a preference for those closer to 
